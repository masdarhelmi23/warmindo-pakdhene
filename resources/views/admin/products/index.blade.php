...
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 md:mb-10 gap-4">
        <div>
            <h1 class="text-2xl md:text-4xl font-extrabold text-slate-900 tracking-tight">
            ...
            <p class="mt-1 md:mt-2 text-xs md:text-sm text-slate-500 font-medium italic">Kelola daftar hidangan, harga, dan ketersediaan stok Warmindo.</p>
        </div>
        
        {{-- BUTTON GROUP --}}
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 md:gap-4">
            <a href="{{ route('categories.index') }}" 
               class="inline-flex items-center justify-center px-5 py-3.5 md:py-4 bg-white border-2 border-slate-200 hover:border-orange-500 text-slate-700 hover:text-orange-600 font-bold rounded-2xl shadow-sm text-xs md:text-sm transition-all duration-300 transform hover:-translate-y-1">
            ...
            <a href="{{ route('products.create') }}" 
               class="inline-flex items-center justify-center px-6 py-3.5 md:py-4 bg-slate-900 hover:bg-orange-600 text-white font-bold rounded-2xl shadow-xl shadow-slate-200 text-xs md:text-sm transition-all duration-300 transform hover:-translate-y-1">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 md:mb-8">
        <div class="bg-white p-5 md:p-6 rounded-2xl border border-slate-100 shadow-sm border-l-4 border-l-orange-500">
            <p class="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest">Total Produk</p>
            <p class="text-xl md:text-2xl font-black text-slate-800 mt-1">
                {{ $products->count() }} <span class="text-xs md:text-sm font-medium text-slate-400 italic">Item</span>
            </p>
        </div>
    </div>

    {{-- 1. TAMPILAN MOBILE: Berupa List Card Vertikal (Hanya muncul di HP, tersembunyi di Desktop) --}}
    <div class="block md:hidden space-y-4">
        ...
        <div class="bg-white rounded-3xl p-5 border border-slate-100 shadow-sm flex items-center justify-between gap-4">
            <div class="flex items-center min-w-0 flex-1">
                {{-- Gambar Menu Mini --}}
                <div class="w-14 h-14 rounded-2xl overflow-hidden shadow-md shadow-orange-100 mr-3.5 bg-slate-50 border border-slate-100 flex-shrink-0">
                    @if($p->image)
                        <img src="{{ asset('storage/' . $p->image) }}" class="w-full h-full object-cover" alt="{{ $p->name }}">
                    @else
                        <div class="w-full h-full gradient-orange flex items-center justify-center text-white font-black text-sm">
                            {{ substr($p->name, 0, 1) }}
                        </div>
                    @endif
                </div>
                ...
                <div class="min-w-0 flex-1">
                    <span class="block font-black text-slate-800 text-base truncate uppercase tracking-tight leading-tight">{{ $p->name }}</span>
                    <div class="flex items-center gap-2 mt-1.5 flex-wrap">
                        <span class="text-[9px] font-bold text-slate-400">STOK: <span class="{{ $p->stock <= 5 ? 'text-red-600 font-black' : 'text-slate-700' }}">{{ $p->stock }}</span></span>
                        <span class="text-[9px] font-black text-orange-600 bg-orange-50 px-2 py-0.5 rounded-md border border-orange-100 uppercase">
                            {{ $p->category->name ?? '-' }}
                        </span>
                    </div>
                    <span class="block text-[11px] font-black text-slate-900 tracking-tight mt-1.5">
                        <span class="text-orange-500 text-[10px] font-bold">Rp</span> {{ number_format($p->price, 0, ',', '.') }}
                    </span>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-2 flex-shrink-0">
                <a href="{{ route('products.edit', $p->id) }}" 
                   class="p-2.5 bg-blue-600 text-white rounded-xl shadow-md shadow-blue-200 flex items-center justify-center active:scale-95 transition-transform">
                ...
                <form action="{{ route('products.destroy', $p->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus menu ini?')" class="m-0 inline">
                    @csrf @method('DELETE')
                    <button type="submit" class="p-2.5 bg-red-600 text-white rounded-xl shadow-md shadow-red-200 flex items-center justify-center active:scale-95 transition-transform">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    {{-- 2. TAMPILAN DESKTOP: Berupa Tabel Utuh (Otomatis sembunyi di HP via 'hidden md:block') --}}
    <div class="hidden md:block bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/50 overflow-hidden border border-slate-100">
    ...
    @if($products->isEmpty())
    <div class="bg-white rounded-3xl py-20 text-center border border-slate-100 shadow-sm mt-4">
        <div class="bg-slate-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 border-2 border-dashed border-slate-200">
        ...
        <p class="text-slate-400 font-bold text-base uppercase tracking-widest">Belum ada data menu</p>
        <p class="text-slate-300 mt-2 text-xs italic">Klik tombol "Tambah Menu Baru" untuk memulai operasional.</p>
    </div>
    @endif
